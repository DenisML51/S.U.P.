...
# --- ДАННЫЕ ДЛЯ ЗАПОЛНЕНИЯ (Без изменений) ---
# ... (оставляем weapons_data, armors_data, и т.д. как были) ...
# --- Оружейные (Примеры) ---
weapons_data = [
...
def seed_data():
    ...
    try:
        ...
        for data in abilities_data:
            # ... (логика добавления способностей осталась без изменений) ...
             ...
             for name in names_to_add:
                 ...
                 if not exists:
                     ...
                     defaults["action_type"] = data_copy.get("action_type", "Действие")
                     defaults["skill_requirements"] = data_copy.get("skill_requirements")

                     ability_fields = {k: v for k, v in data_copy.items() if k in Ability.__table__.columns or k == 'skill_requirements'}
                     ability_fields.update({k: v for k, v in defaults.items() if v is not None}) # Добавляем/перезаписываем только не None значения по умолчанию

                     ability_fields_cleaned = {k:v for k,v in ability_fields.items() if k in Ability.__mapper__.attrs or k == 'skill_requirements'} # Оставляем только то, что есть в модели
        ...
        for data in status_effects_data:
            # ... (логика добавления эффектов осталась без изменений) ...
            names_to_add = split_names(data["name"])
            for name in names_to_add:
                data_copy = data.copy()
                data_copy["name"] = name
        ...
        for data in weapons_data:
            # ... (логика добавления оружия осталась без изменений) ...
            names_to_add = split_names(data["name"])
            ...
            for name in names_to_add:
                ...
                if not exists:
                    ...
                    for ability_name in granted_ability_names:
                        ...
                        else: print(f"    Warning: Ability '{ability_name}' not found for weapon '{name}'.")
                    data_copy.pop('granted_abilities', None)
                    item_defaults = {"category": "Простое", "rarity": "Обычная", "weight": 1, "strength_requirement": 0, "stealth_disadvantage": False}
                    data_with_defaults = {**item_defaults, **data_copy}
                    try:
                        ...
                        print(f"  Added Weapon: {name}")
                         ...
                         print(f"    Data used: {data_with_defaults}")
                else: print(f"  Skipped Weapon (exists): {name}")

        print("\nSeeding Armor...")
        for data in armors_data:
            # ... (логика добавления брони осталась без изменений) ...
            names_to_add = split_names(data["name"])
            for name in names_to_add:
                data_copy = data.copy()
                data_copy["name"] = name
                ...
                if not exists:
                    ...
                    if max_dex is not None:
                        ...
                        except (ValueError, TypeError): data_copy["max_dex_bonus"] = None
                    item_defaults = {"category": "Простое", "rarity": "Обычная", "weight": 1}
                    # Убедимся, что обязательные для модели Armor поля имеют значения по умолчанию, если не заданы
                    data_with_defaults = {
                         "strength_requirement": 0,
                         "stealth_disadvantage": False,
                          **item_defaults, **data_copy
                    }
                    try:
                        item = Armor(**data_with_defaults)
                        db.add(item)
        ...
        for data in shields_data:
            # ... (логика добавления щитов осталась без изменений) ...
            names_to_add = split_names(data["name"])
            for name in names_to_add:
                data_copy = data.copy()
                data_copy["name"] = name
                ...
                if not exists:
                    item_defaults = {"category": "Простое", "rarity": "Обычная", "weight": 1}
                    data_with_defaults = {
                        "strength_requirement": 0, # Обязательно для модели Shield
                        **item_defaults, **data_copy
                    }
                    try:
                        item = Shield(**data_with_defaults)
                        db.add(item)
        ...
        for data in general_items_data:
            # ... (логика добавления общих предметов осталась без изменений) ...
            names_to_add = split_names(data["name"])
            for name in names_to_add:
                data_copy = data.copy()
                data_copy["name"] = name
                ...
                if not exists:
                    ...
                    data_with_defaults = {**item_defaults, **data_copy}
                    try:
                        item = GeneralItem(**data_with_defaults)
                        db.add(item)
        ...
        for data in ammo_data:
            # ... (логика добавления патронов осталась без изменений) ...
             names_to_add = split_names(data["name"])
             for name in names_to_add:
                 data_copy = data.copy()
                 data_copy["name"] = name
                 exists = db.query(Item.id).filter(Item.name == name).first()
                 if not exists:
                     item_defaults = {"category": "Простое", "rarity": "Обычная", "weight": 0}
                     data_with_defaults = {**item_defaults, **data_copy}
                     try:
                         item = Ammo(**data_with_defaults)
                         db.add(item)
