...
const LevelUpModal = ({ characterId, currentCharacterData, onClose, onLevelUpSuccess }) => {
    ...
    const pointsLeft = 3 - pointsSpent;

    const handleSkillChange = (skillName, change) => {
        const currentAddedPoints = skillPoints[skillName];
        const currentSkillLevel = currentCharacterData?.[skillName] || 1; // Безопасный доступ
        let newAddedPoints = currentAddedPoints + change;

        if (change > 0 && pointsLeft <= 0) return;
        if (change < 0 && newAddedPoints < 0) newAddedPoints = 0;

        const projectedLevel = currentSkillLevel + newAddedPoints;
        if (projectedLevel > 10) { setError(`Навык ${skillName.replace('skill_', '')} не может превышать 10.`); return; }
        const projectedTotalSpent = pointsSpent - currentAddedPoints + newAddedPoints;
         if (projectedTotalSpent > 3) { setError("Можно распределить только 3 очка навыков."); return; }
        setError("");
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        setError("");
        if (hpRoll === '' || hpRoll < 1 || hpRoll > 10) { setError("Введите корректный результат броска HP (1-10)."); return; }
        if (!selectedBranch) { setError("Выберите ветку класса для прокачки."); return; }
        if (pointsSpent !== 3) { setError("Необходимо распределить ровно 3 очка навыка."); return; }
        const branchLevelKey = `${selectedBranch}_branch_level`;
        if (!currentCharacterData || typeof currentCharacterData[branchLevelKey] !== 'number') { setError("Ошибка данных персонажа. Не удалось проверить уровень ветки."); return; }
        if (currentCharacterData[branchLevelKey] >= 10) { setError(`Ветка "${ALL_BRANCHES.find(b => b.key === selectedBranch)?.name}" уже достигла максимального уровня (10).`); return; }

        setIsLoading(true);
        const skillPointsSpentData = {};
        for (const skill in skillPoints) { if (skillPoints[skill] > 0) skillPointsSpentData[skill] = skillPoints[skill]; }
        const levelUpData = { hp_roll: parseInt(hpRoll, 10), branch_point_spent: selectedBranch, skill_points_spent: skillPointsSpentData };
        ...
        try {
            ...
            console.error("Level up error:", err);
             // Обрабатываем ошибку валидации с бэкенда более детально
            ...
            <div style={styles.modal}>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div style={styles.formGroup}>
                        ...
                        <input style={styles.input} type="number" id="hpRoll" min="1" max="10" value={hpRoll} onChange={(e) => setHpRoll(Number(e.target.value))} required placeholder="1-10"/>
                    </div>
                    ...
                    <div style={styles.formGroup}>
                        ...
                        <div style={styles.branchGrid}>
                            {ALL_BRANCHES.map(branch => {
                                ...
                                return (
                                    <button
                                        type="button" key={branch.key} onClick={() => handleBranchSelect(branch.key)}
                                        style={{ ...styles.branchCard, ...(isSelected ? styles.branchCardSelected : {}), ...(isDisabled ? styles.branchCardDisabled : {}) }}
                                        disabled={isDisabled || isLoading}
                                        title={isDisabled ? `${branch.name} - Макс. уровень (${currentLevel})` : `Выбрать ${branch.name} (${currentLevel} ур.)`}
                                    >
                        ...
                        {!selectedBranch && <p style={styles.hintText}>Выберите ветку</p>}
                    </div>

                    {/* Распределение Очков Навыков */}
                    <div style={styles.formGroup}>
                        <label style={styles.label}>Очки Навыков (+3 очка):</label>
                        <p style={styles.pointsInfo}>Осталось распределить: <strong style={{ color: pointsLeft === 0 ? theme.colors.secondary : theme.colors.primary }}>{pointsLeft}</strong> / 3</p>
                            ...
                            {ALL_SKILLS.map(skillName => {
                                ...
                                return (
                                    <div key={skillName} style={styles.skillItem}>
                                        ...
                                        <span style={styles.skillLevelValue}>{currentLevel + addedPoints}</span>
...
const styles = {
    ...
    input: { width: '100%', padding: '10px 12px', borderRadius: '8px', border: `1px solid ${theme.colors.textSecondary}`, background: 'rgba(255, 255, 255, 0.1)', color: theme.colors.text, fontSize: '1rem', boxSizing: 'border-box' },
