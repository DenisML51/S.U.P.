...
// --- Иконки для Деталей ---
// (Можно вынести в отдельный файл с иконками)
...
// Иконки для требований (можно добавить или использовать текст)
// const CheckIcon = () => (<svg style={styles.reqIcon} viewBox="0 0 24 24" fill="currentColor"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>);
// const CrossIcon = () => (<svg style={styles.reqIcon} viewBox="0 0 24 24" fill="currentColor"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg>);
...
const AbilityDetailModal = ({ ability, onClose, character }) => {
    ...
    // --- Обработка Требований Навыков ---
    const requirementsMet = useMemo(() => {
        // Проверяем ability *внутри* useMemo
        if (!ability?.skill_requirements || !character?.skills) return { met: true, details: {} };
        ...
        try {
            ...
            if (typeof ability.skill_requirements === 'string' && ability.skill_requirements.trim()) {
                ...
                return { met: true, details: {} }; // Пустые требования считаем выполненными
            }
            // Проверяем, что requirements это объект
    ...
    }, [ability?.skill_requirements, character?.skills]);
        ...
        <>
            ...
            <div style={styles.overlay} onClick={onClose}>
                <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
                    ...
                    {/* Проверяем, что details не содержит только ключ 'error' */}
                    {requirementsMet.details && Object.keys(requirementsMet.details).length > 0 && !requirementsMet.details.error && (
                        <div style={styles.requirementsSection}>
                             ...
                             <div style={styles.tagsContainerReq}> {/* Отдельный контейнер для тегов требований */}
                                 {Object.entries(requirementsMet.details).map(([key, val]) => {
                                     ...
                                     const met = val.met;
                                     return (
                                         <span key={key} style={{...styles.detailTag, ...(met ? styles.reqMet : styles.reqNotMet)}} title={met ? `Выполнено (у вас ${val.current})` : `Нужно ${val.required} (у вас ${val.current})`}>
                                             {skillName} {val.required} {met ? '✓' : '✕'}
                                         </span>
                                     );
                                 })}
                             </div>
                     ...
                     {/* Отображаем ошибку требований, если она есть */}
...
const styles = {
    ...
    title: { textAlign: 'center', marginBottom: '20px', color: theme.colors.secondary, borderBottom: `1px solid ${theme.colors.secondary}55`, paddingBottom: '15px', fontSize: '1.5rem', fontWeight: '600' },
    ...
    tagsContainerReq: { display: 'flex', flexWrap: 'wrap', gap: '8px', alignItems: 'center', marginTop: '8px' }, // Отдельный контейнер для тегов требований
    reqIcon: { width: '14px', height: '14px', fill: 'currentColor', opacity: 0.8, marginRight: '2px' }, // Используется в subHeader
    ...
    buttonGroup: { display: 'flex', justifyContent: 'flex-end', marginTop: 'auto', paddingTop: '20px', borderBottom: `1px solid ${theme.colors.surfaceVariant}` }, // Исправил borderTop на borderBottom у предыдущего элемента
