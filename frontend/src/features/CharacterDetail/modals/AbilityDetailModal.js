...
// Импортируем StatDisplay, если он нужен внутри модалки
// import StatDisplay from '../../../components/UI/StatDisplay'; // Если StatDisplay используется

// Вспомогательный компонент для строки стат ТОЛЬКО для этой модалки
// Если такой же используется где-то еще, выносим в components/UI
...
const AbilityDetailModal = ({ ability, onClose }) => {
    ...
    if (ability.skill_requirements) {
        try {
            // Проверяем, строка ли это и не пустая ли
            if (typeof ability.skill_requirements === 'string' && ability.skill_requirements.trim()) {
                ...
                requirementsDetails = Object.entries(parsedReqs)
                                          .map(([key, value]) => `${key.replace('skill_', '')} ${value}`) // Убираем 'skill_'
                                          .join(', ');
            } else {
                 requirementsDetails = "-"; // Или null, если не хотим показывать поле
            }
        } catch (e) {
        ...
        <div style={styles.overlay} onClick={onClose}>
            ...
            <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
                ...
                <div style={styles.detailsGrid}>
                    ...
                    {requirementsDetails && requirementsDetails !== "-" && <StatRow label="Треб. Навыков" value={requirementsDetails} />}
